...
if __name__ == "__main__":
    ...
    if param == 'core':
        ...
        [num_benchs, num_exps, num_vms, vms_exps_benchs] = vmm.pre_draw(data_dir, [None] * 1)
        num_figs = 1
        xlabels = ['Number of Threads', 'Number of Threads', 'Frequency(MHz)']
        ylabels = ['Frequency(MHz)', 'Run Time(s)', 'Run Time(s)']
        xaxis = [range(0, 80, 8), range(0, 80, 8), None]
        ...
        for id_bench in range(0, 1):
            ...
            for id_exp in range(0, num_exps):
                for id_vm in range(0, 2):
                    ...
                    ele.print()
            for id_vm in range(0, 2):
                axs[0].plot(num_cores[id_vm], bzy_freq[id_vm], label = vms_exps_benchs[id_bench][0][0].bench_name)
        vmm.post_draw(num_figs, axs)
